...
const SectorBreakdown = () => {
  ...
  const sectorBreakdown = useMemo((): SectorData[] => {
    ...
    const totalValue = sampleData.reduce((sum, holding) => sum + holding.presentValue, 0);
  ...
  // Get holdings for selected sector
    ...
    <div className=" bg-gradient-to-br from-gray-50 to-blue-50 text-black p-6 pt-24">
      <div className="max-w-6xl mx-auto">
        <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Portfolio Sector Breakdown</h1>
          <p className="text-gray-600 mb-6">Analysis of your investments across different sectors</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              ...
              <p className="text-2xl font-bold text-blue-900">
                {formatCurrency(sampleData.reduce((sum, holding) => sum + holding.presentValue, 0))}
              </p>
            </div>
            <div className="bg-green-50 p-4 rounded-lg border border-green-100">
              <h3 className="text-sm font-medium text-green-800">Number of Holdings</h3>
              <p className="text-2xl font-bold text-green-900">{sampleData.length}</p>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-100">
              <h3 className="text-sm font-medium text-purple-800">Sectors</h3>
              <p className="text-2xl font-bold text-purple-900">{sectorBreakdown.length}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="relative flex items-center justify-center mb-6">
                <div className="relative w-56 h-56">
                  ...
                  {sectorBreakdown.map((sector, i, array) => {
                    const offset = array.slice(0, i).reduce((sum, s) => sum + (s.value / 100) * 360, 0);
                    return (
                      <div
                        key={sector.name}
                        className="absolute inset-0"
                        style={{
                          clipPath: `conic-gradient(from ${offset}deg, ${sector.color} 0 ${sector.value * 3.6}deg, transparent ${sector.value * 3.6}deg)`
                        }}
                      ></div>
                    );
                  })}
                ...
                <div className="absolute flex flex-col items-center justify-center">
                  <span className="text-2xl font-bold">{sectorBreakdown.length}</span>
                ...
                {sectorBreakdown.map(sector => (
                  <div 
                    ...
                    className={`flex items-center justify-between p-3 rounded-lg cursor-pointer transition-colors ${
                      selectedSector === sector.name ? 'bg-blue-100' : 'bg-white hover:bg-gray-100'
                    }`}
                  ...
                  >
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-sm text-gray-500 ml-2">({sector.holdings} holdings)</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-gray-50 p-6 rounded-lg">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">
                {selectedSector ? `${selectedSector} Holdings` : "Select a Sector"}
              </h2>
                        ...
                        <tr key={holding.id} className="hover:bg-gray-50">
                          <td className="px-4 py-4 whitespace-nowrap">
                            <div className="flex flex-col">
                              <span className="font-medium">{holding.particulars}</span>
                              <span className="text-sm text-gray-500">{holding.exchange}</span>
                            </div>
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap">
                    ...
                    <tfoot className="bg-gray-50">
                      <tr>
                        ...
                        <td className="px-4 py-4 font-medium">
                          {formatCurrency(sectorHoldings.reduce((sum, holding) => sum + holding.presentValue, 0))}
                        </td>
                        <td className={`px-4 py-4 font-medium ${
                          sectorHoldings.reduce((sum, holding) => sum + holding.gainLoss, 0) >= 0 
                            ? 'text-green-600' 
                            : 'text-red-600'
                        }`}>
                          {sectorHoldings.reduce((sum, holding) => sum + holding.gainLoss, 0) >= 0 ? '▲' : '▼'} 
                          {' '}
                          {formatCurrency(Math.abs(sectorHoldings.reduce((sum, holding) => sum + holding.gainLoss, 0)))}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
        ...
        <div className="bg-white rounded-xl shadow-lg p-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4">Performance by Sector</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {sectorBreakdown.map(sector => {
              const sectorData = sampleData.filter(h => h.sector === sector.name);
              const totalGainLoss = sectorData.reduce((sum, h) => sum + h.gainLoss, 0);
              const totalInvestment = sectorData.reduce((sum, h) => sum + h.investment, 0);
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {formatCurrency(sectorData.reduce((sum, h) => sum + h.presentValue, 0))}
                      </span>
                    </div>
                    <div className="flex justify-between">
